    ...
    /******************************************************/
{
    ...
    remove(fname);               /* Delete old file if it already exists */
    ...
    /*******************************************************************/
{
    ...
    char fname[] = "atestfil.fit";           /* name for new FITS file */
...
void copyhdu( void)
{
    ...
    char infname[]  = "atestfil.fit";  /* name for existing FITS file   */
    char outfname[] = "btestfil.fit";  /* name for new FITS file        */

    int status, morekeys, hdutype;

    status = 0;

    remove(outfname);            /* Delete old file if it already exists */

    /* open the existing FITS file */
    if ( fits_open_file(&infptr, infname, READONLY, &status) ) 
         printerror( status );

    if (fits_create_file(&outfptr, outfname, &status)) /*create FITS file*/
         printerror( status );           /* call printerror if error occurs */
    ...
    /*********************************************************************/
{
    ...
    if ( fits_open_file(&infptr,  infname,  READONLY,  &status) ||
         fits_open_file(&outfptr, outfname, READWRITE, &status) ) 
         printerror( status );
...
/*    char fname[]  = "atestfil.fit"; */     /* name of existing FITS file   */
